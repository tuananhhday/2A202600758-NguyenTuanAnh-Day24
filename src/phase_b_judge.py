...
def swap_and_average(question: str, answer_a: str, answer_b: str) -> JudgeResult:
    ...
    winner_pass2 = swap_map[pass2_raw.get("winner", "tie")]
    ...
    return JudgeResult(
        ...
        scores_pass1=pass1.get("scores", {"A":0,"B":0}),
        scores_pass2={"A": pass2_raw.get("scores", {}).get("B", 0), "B": pass2_raw.get("scores", {}).get("A", 0)},
    )
...
if __name__ == "__main__":
    ...
    # Run judge on the 10 questions to get actual judge_labels
    judge_results = []
    judge_labels = []
    for item in human_data:
        res = swap_and_average(item["question"], item["model_answer"], "Không có thông tin trong tài liệu.")
        judge_results.append(res)
        label = 1 if res.final_winner == "A" else 0
    ...
    # --- Bias report ---
    bias = bias_report(judge_results + [result])
    print(f"\nBias report: {bias}")
