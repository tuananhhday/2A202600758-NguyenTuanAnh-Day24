...
def run_query(query: str, search: HybridSearch, reranker: CrossEncoderReranker) -> tuple[str, list[str]]:
    ...
    contexts = []
    for r in (reranked or results[:3]):
        parent_text = r.metadata.get("parent_text") if r.metadata else None
        if parent_text:
            contexts.append(parent_text)
        else:
            contexts.append(r.text)

    from config import OPENAI_API_KEY as LLM_KEY
    LLM_MODEL = "gpt-4o-mini"
    def create_llm_client():
        from openai import OpenAI
        return OpenAI()
    if LLM_KEY and contexts:
        try:
